...
# -----------------------------------------------------------------------------
# Bypass SSL certificate validation for self-signed certs (NET::ERR_CERT_AUTHORITY_INVALID)
# Uses ServerCertificateValidationCallback which is the reliable method in PS 5.1
# -----------------------------------------------------------------------------
function Disable-SSLValidation {
    # Callback approach - works reliably with Invoke-RestMethod in PowerShell 5.1
    [System.Net.ServicePointManager]::ServerCertificateValidationCallback = { $true }

    # Also set the security protocol to allow TLS 1.0 / 1.1 / 1.2
    # Some older MP508 firmware only supports TLS 1.0 or 1.1
...
# Strip http:// or https:// prefix if present, keep optional :port suffix
# Accepts any of these formats in the IP list file:
#   192.168.1.100
#   192.168.1.100:443
#   https://192.168.1.100
...
# Use Read-Host instead of Get-Credential so Windows SSO / credential manager
# cannot auto-fill the fields. The user must type both values manually.
...
if ([string]::IsNullOrWhiteSpace($Username)) {
    Write-Host ""
    Write-Host "  ERROR: Username cannot be blank. Exiting." -ForegroundColor Red
    exit 1
}
...
if ([string]::IsNullOrWhiteSpace($Password)) {
    Write-Host ""
    Write-Host "  ERROR: Password cannot be blank. Exiting." -ForegroundColor Red
    exit 1
}
